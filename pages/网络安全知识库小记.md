...
编译后的二进制文件位于 `target/release/cybersecurity-knowledge-base`

### 运行

```bash
./target/release/cybersecurity-knowledge-base
```

### 系统服务 (systemd)

创建 `/etc/systemd/system/cybersec-kb.service`：
...
Type=simple
User=cybersec
WorkingDirectory=/opt/cybersec-kb
ExecStart=/opt/cybersec-kb/cybersecurity-knowledge-base
Restart=always
Environment=RUST_LOG=info
Environment=NVD_API_KEY=your_api_key
...
```bash
sudo systemctl enable cybersec-kb
sudo systemctl start cybersec-kb
```
...
COPY --from=builder /app/target/release/cybersecurity-knowledge-base /usr/local/bin/
COPY --from=builder /app/static /app/static
COPY --from=builder /app/templates /app/templates
WORKDIR /app
EXPOSE 3000
CMD ["cybersecurity-knowledge-base"]
```

构建和运行：

```bash
docker build -t cybersec-kb .
docker run -p 3000:3000 -v $(pwd)/data:/app cybersec-kb
```
